...
bot.on("message", msg => {
    ...
    if (msg.content.startsWith(".phraseadd")) {
        ...
        var mode;
        if (file === 'noco')
            mode = './server/Clippy.txt';
        else if (file === 'erp')
            erp = './server/erp.txt'
        else
            msg.channel.send('No file specified');
        addLine(mode, phrase, function(res) {
    ...
    if (msg.content.startsWith(".create_giveaway")) {
        ...
        const collector = bot.channels.get("257525917876748289").createCollector(
            m => m.content.startsWith(".num"), { maxMatches: 500, time: timeleft }
        );
...
function addLine(filename, phrase, cb) {
    fs.appendFile(filename, phrase, function(err) {
